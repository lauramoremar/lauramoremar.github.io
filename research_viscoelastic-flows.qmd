...
Viscoelastic fluids are materials that exhibit both viscous and elastic behavior, making their modeling particularly challenging, especially at high Weissenberg numbers,  that is, for highly elastic flows.
...
Note that these research topics were a natural continuation of the Ernesto Castillo's [thesis](http://hdl.handle.net/2117/96306).


## What is a Viscoelastic Fluid?

Viscoelastic fluids are a specific type of **non-Newtonian fluid** that exhibit a combination of both **viscous** and **elastic** behaviors:

- **Viscous**: associated with friction, energy dissipation, and loss of memory.
- **Elastic**: characterized by the ability to recoil and store internal energy.

This unique combination stems from their **complex internal microstructure**, which leads to *memory effects*:
...
- These materials require a **nonlinear constitutive equation** to accurately describe their behavior, going beyond the classical Navier–Stokes framework.
- Their complex rheology poses significant challenges for standard numerical methods, especially at high elasticity.

## Modelling of polymeric fluid flows


While Newtonian fluids can be modeled using only the Navier–Stokes equations (which include the momentum and continuity equations), modeling viscoelastic fluid flows requires an additional constitutive equation to properly capture the elastic behavior of the fluid.
